...
function log(level, message, data = null) {
    ...
    const logFile = path.join(__dirname, 'logs', 'multiplayer.log');
...
function createMatch(modeKey, clientsForMatch) {
  ...
  clientsForMatch.forEach((client, index) => {
    // Try to balance teams based on skill level (if available)
  ...
  log('INFO', 'Match created', { 
    ...
    mode: modeKey, 
    map: mapName, 
    players: clientsForMatch.length,
    teams: { A: teamA.length, B: teamB.length }
  });
...
function maybeCreateMatch(modeKey) {
  ...
  // Enhanced matchmaking: Try to fill matches as soon as possible
...
function queueClient(client, modeKey) {
  ...
  log('INFO', 'Client queued', { 
    clientId: client.id, 
    mode: modeKey, 
    queueSize: queues.get(modeKey).length,
    required: MODES[modeKey].playersPerTeam * 2
  });
...
app.get('/health', (req, res) => {
  res.json({ 
    ...
    queues: Object.fromEntries([...queues.entries()].map(([mode, queue]) => [mode, queue.length])),
...
app.post('/api/multiplayer/coach', async (req, res) => {
  ...
  if (!aiCoach) {
    log('WARN', 'AI coach unavailable, using fallback', { mode, map, loadout });
  ...
  try {
    ...
    log('INFO', 'AI coach response generated', { mode, map, adviceCount: parsed.advice.length });
    res.json({ source: 'gemini', advice: parsed.advice.slice(0, 4) });
  } catch (error) {
    log('ERROR', 'AI coach error', { error: error.message });
    res.json({ source: 'fallback', ...fallback });
  }
});
...
wss.on('connection', socket => {
  ...
  log('INFO', 'Client connected', { clientId: client.id, ip: socket._socket.remoteAddress });
  ...
  socket.on('message', raw => {
    ...
    try {
      ...
      log('WARN', 'Invalid message received', { clientId: client.id, error: error.message });
      return;
    }
    ...
    if (message.type === 'profile') {
      ...
      log('INFO', 'Profile updated', { clientId: client.id, name: client.name, preferences: client.preferences });
    }
    ...
    if (message.type === 'leave-queue') {
      removeClientFromQueues(client.id);
      log('INFO', 'Client left queue', { clientId: client.id });
    }
  ...
  socket.on('close', () => {
    ...
    if (client.matchId && matches.has(client.matchId)) {
      ...
      if (match.players.length < 2) {
        log('INFO', 'Match ended due to insufficient players', { matchId: client.matchId });
        matches.delete(match.id);
      }
    }
  ...
  socket.on('error', (error) => {
    log('ERROR', 'Socket error', { clientId: client.id, error: error.message });
  });
});

server.listen(PORT, () => {
  log('INFO', 'Multiplayer server started', { 
    port: PORT, 
  ...
  console.log(`🔌 API: http://localhost:${PORT}/api/multiplayer`);
